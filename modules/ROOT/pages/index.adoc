...
- `type` (string) -- `text`.
- `name` (string) -- a human-friendly label for the source. You will use this to reference the source in the targets section.
The name must be unique across all the sources of the job specification file.
...
- `**source**` (string) -- the name of the data source this target should draw data from.
The data source must be defined in this job specification file.
- `name` (string) -- a human-friendly name for the target.
The name must be unique across all the targets of this job specification file.
...
- **make sure to quote and escape labels**. If you don't quote a label, it is considered as a dynamic value, which is interpreted as the corresponding column value of the current row when running the Dataflow job.
...
- **make sure to quote and escape relationship types and node labels**. If you don't quote constants, they are considered as dynamic values, which is interpreted as the corresponding column value of the current row when running the Dataflow job.
- if `index_all_properties: true` in config, it is pointless to specify any columns in `properties.indexed`.
...
- `limit` (int) -- append a `LIMIT` clause (here `LIMIT 50`) to the generated SQL query that fetches the source data (defaults to no limit, encoded into -1).
...
* `preloads` -- after all actions executing after `start` have run
