...
You can script execution of Dataflow jobs using the link:https://cloud.google.com/dataflow/docs/guides/templates/using-flex-templates[Google Cloud CLI]. Executing a job through the CLI is functionally equivalent to using the Dataflow user interface.
...
If the credentials file is stored in Google Secret Manager, the parameter `neo4jConnectionUri` is replaced by `neo4jConnectionSecretId` and the invocation becomes:
