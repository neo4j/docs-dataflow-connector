= Run an import job (Google Cloud CLI)
...
[TIP]
Add `--service-account-email` to run the job as a dedicated service account rather than the Compute Engine default one, which holds the broad `roles/editor` on the project.
xref:least-privilege-setup.adoc#single-project[Least-privilege setup examples] walks through creating such an account with only the roles an import needs.
