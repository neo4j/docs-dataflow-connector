...
| Constructs the https://docs.cloud.google.com/dataflow/docs/concepts/beam-programming-model[Apache Beam®] Neo4j import pipeline. No connectivity to sources or to Neo4j is needed at this stage.

| Worker VMs
| The worker service account
| Reads the job specification from Cloud Storage. Resolves the Neo4j connection metadata, from Secret Manager or Cloud Storage. Connects to Neo4j to verify connectivity, apply `reset_db` if applicable, and create all constraints and indexes.
Then reads the source data and connects to Neo4j to write nodes and relationships.
|===
