...
** `**keys**` (list of objects) -- source columns that should be mapped into node properties _and_ get a node key constraint. These get mapped to string properties.
** `mandatory` (list of objects) -- source columns that should be mapped into node properties _and_ get a node property existence constraint. These get mapped to string properties.
** `unique` (list of objects) -- source columns that should be mapped into node properties _and_ get a unique node property constraint. These get mapped to string properties.
** `indexed` (list of objects) -- source columns that should be mapped into node properties _and_ get an index on the corresponding node property. These get mapped to string properties.
** `strings`, `floats`, `ints`, `dates`, `points`, `booleans` (list of objects) -- source columns that should be mapped into node properties of the given type. The data type affects how the data is represented into Neo4j, but does not create type constraints.
...
** `mandatory` (list of objects) -- source columns that should be mapped into relationship properties _and_ get a relationship property existence constraint. These get mapped to string properties.
** `unique` (list of objects) -- source columns that should be mapped into relationship properties _and_ get a relationship uniqueness constraint. These get mapped to string properties.
** `indexed` (list of objects) -- source columns that should be mapped into relationship properties _and_ get an index on the corresponding relationship property. These get mapped to string properties.
** `strings`, `floats`, `ints`, `dates`, `points`, `booleans` (list of objects) -- source columns that should be mapped into node properties of the given type. The data type affects how the data is represented into Neo4j, but does not create type constraints.
