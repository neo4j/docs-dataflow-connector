...
- `start_node_reference` (string) -- The name of the node target that acts as _start_ for the relationship. `start_node` can play the same role, except it takes a xref:_node_objects[full node object].
- `end_node_reference` (string) -- The name of the node target that acts as _end_ for the relationship. `end_node` can play the same role, except it takes a xref:_node_objects[full node object].
