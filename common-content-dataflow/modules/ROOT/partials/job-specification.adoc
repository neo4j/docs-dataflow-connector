...
- `index_all_properties` (bool) -- Whether to create range indexes on all imported properties.
Defaults to `false`.
...
[WARNING]
====
Enabling `index_all_properties` creates a range index on every imported property.
This increases import time, write overhead, and index storage, and can lead to index bloat on large datasets.
Enable it only when you need broad query coverage.
====
