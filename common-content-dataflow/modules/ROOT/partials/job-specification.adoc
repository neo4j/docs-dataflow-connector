...
- `start_node_reference` (object) -- The name of the node target that acts as _start_ for the relationship, together with the column name that acts as key in the source (`source_field`) and the imported property that acts as key in the node target (`node_property`).
- `end_node_reference` (object) -- The name of the node target that acts as _end_ for the relationship, together with the column name that acts as key in the source (`source_field`) and the imported property that acts as key in the node target (`node_property`).
