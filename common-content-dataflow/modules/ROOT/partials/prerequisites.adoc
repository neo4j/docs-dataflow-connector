...
[TIP]
Secret Manager is the better choice for anything but experimentation, since the Cloud Storage bucket needs broad write access for staging and every principal that can read it can then read your Neo4j credentials.
...
See xref:permissions.adoc[Permissions] for the roles the job needs, and for a cross-project setup using service-account impersonation.
// end::dataflow-job[]
