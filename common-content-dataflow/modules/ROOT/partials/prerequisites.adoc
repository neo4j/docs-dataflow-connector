...
- sign up for link:https://neo4j.com/cloud/aura/[Neo4j Aura]
- install and self-host Neo4j (see link:https://neo4j.com/docs/operations-manual/current/installation/[Neo4j -> Installation]) with port 7687 open (Bolt protocol)
...
[TIP]
Secret Manager is the best choice for anything but experimentation, since the Cloud Storage bucket needs broad write access for staging and every principal that can read it can then read your Neo4j credentials.
Either way, the job's service account needs an explicit permission to read the file -- see xref:permissions.adoc#secret-manager[Permissions].
...
See xref:permissions.adoc[Permissions] for the roles the job needs, and xref:least-privilege-setup.adoc#cross-project-impersonation[Least-privilege setup examples] for a cross-project setup using service-account impersonation.
// end::dataflow-job[]
