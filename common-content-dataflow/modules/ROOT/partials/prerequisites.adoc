...
- install and self-host Neo4j (see link:https://neo4j.com/docs/operations-manual/current/installation/[Neo4j -> Installation])

[IMPORTANT]
====
Connectivity requirements depend on how your Neo4j instance is deployed:

*Aura*::
Use the connection URI from the Aura console (`neo4j+s://…`).
Some template constraint types require Aura Enterprise.

*Self-managed*::
Dataflow workers must be able to reach your Neo4j instance on the Bolt port (default `7687`).
The instance does not need to be publicly accessible.
====
...
// tag::gcs-bucket-bigquery[]
== Google Cloud Storage bucket

You need a link:https://console.cloud.google.com/storage/[Google Cloud Storage] bucket.
This is the only location from which the Dataflow job can read configuration files.
// end::gcs-bucket-bigquery[]
