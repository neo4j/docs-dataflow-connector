...
- `query_temp_project` (string, optional) Specifies the project to store temporary query results, processed by Apache Beam. If not set, Apache
 Beam will use the current project.
- `query_temp_dataset` (string, optional) Specifies the dataset to store temporary query results, processed by Apache Beam. If not set, Apache Beam will create its own datasets for processing.

[IMPORTANT]
If you set query_temp_project, you must also specify query_temp_dataset.
You can specify only query_temp_dataset, and Apache Beam will use the current project.
If neither is set, Apache Beam will use the current project and create its own datasets for temporary processing.
...
----
{
  ...
  "query_temp_project": "temp-project",
  "query_temp_dataset": "temp-dataset"
}
----
